...
const Medicaments = ({navigation}) => {

  const [medicaments, setMedicaments] = useState([]);

  const navigateRegistrarMedicamento = async (medicament = null) => {
    if (!medicament) {
      return navigation.navigate('RegisterMedicament', {
        selectedMedicament: null,
      });
    } else {
      navigation.navigate('RegisterMedicament', {
        selectedMedicament: {
          _id: medicament._id,
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity 
      ...
      onPress={() => navigateRegistrarMedicamento(item)} // Pasa el medicamento seleccionado
      >
      <Image source={{ uri: item.Image }} style={styles.medicamentImage} />
      <Text style={styles.medicamentName}>{item.Name}</Text>
      <Text style={styles.medicamentDose}>{item.Dose}</Text>
      <Text style={styles.medicamentIndications}>{item.Indications}</Text>
    </TouchableOpacity>
  );
    ...
    <View style={styles.container}>
      ...
      />
       ...
       <TouchableOpacity
        ...
        onPress={navigateRegistrarMedicamento}
